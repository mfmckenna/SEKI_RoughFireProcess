...
Outdir = "D:\\SEKI SoundScape\\RoughFire\\ANALYSIS\\AcousticIndex" #where to put the site files with acoustic indices


#startday =  user <- dlgInput("What is the NVSPL start day (year_month_day)", Sys.info()["user"])$res
...
## GET FILES
#___________________________________________________________________________________________________
workingDir = "D:\\SEKI SoundScape\\RoughFire\\NVSPL"
...
#___________________________________________________________________________________________________
for (ss in 9:length(mySites)) # 1:length(mySites) START loop through sites
{
  OUTPUT_sites = NULL
  OUTPUT_sitesD = NULL
